...
class PointEarned(BASE):
    ...
    student_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('students_table.id'), nullable=False)
    student: Mapped[Student] = relationship('Student', primaryjoin='PointEarned.student_id == Student.id')
    event_instance_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('event_instances_table.id'), nullable=False)
    event_instance: Mapped[EventInstance] = relationship('EventInstance', primaryjoin='PointEarned.event_instance_id == EventInstance.id', index=True)
    point_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('points_table.id'), nullable=False)
...
class PointSpent(BASE):
    ...
    event_instance_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('event_instances_table.id'), nullable=False)
    event_instance: Mapped[EventInstance] = relationship('EventInstance', primaryjoin='PointSpent.event_instance_id == EventInstance.id', index=True)
...
class RunningTotal(BASE):
    ...
    id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    student_id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('students_table.id'), nullable=False)
    student: Mapped[Student] = relationship('Student', primaryjoin='RunningTotal.student_id == Student.id', index=True)
